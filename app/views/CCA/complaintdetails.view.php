                        ...
                        <div class="dis-flex gap-10 ju-co-ce al-it-ce pad-20 bor-rad-5 txt-c-black">
                            ...
                            <div class="popup" id="popup">
                                <form method="post"  action="<?= ROOT ?>/cca/complaints/accepted/assists/<?=$comp->comp_id ?>">
                                    ...
                                    <button type="submit" style="background-color: #7b51d3" onclick="closePopup()">Ok</button>
                                </form>
                            </div>
                            <button type="button" class="button-s2"  onclick="openPopup1()">Handle</button>
                            <div class="popup1" id="popup1">
                                <form id="myForm" method="post" action="<?= ROOT ?>/cca/complaints/accepted/handle/<?=$comp->comp_id ?>">
                                    <div class="input-box">
                                        <label >Handled Details</label>
                                        <textarea class="handle_comment" required id="comment" name="comment"> </textarea>
                                    </div>
                                    <button type="submit" class="button-s2" onclick="closePopup1()">Ok</button>
                                </form>
                            </div>
                        </div>
                    <?php endif ?>
                        ...
                        <div class="popup" id="popup">
                            <form method="post"  action="<?= ROOT ?>/cca/complaints/assists/update/<?=$comp->comp_id ?>">
                                <div class="input-box">
                                    ...
                                    <textarea required id="comment" name="comment"><?=$comp->comment ?> </textarea>
                                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.getElementById('myForm').addEventListener('submit', function(event) {
            var comment = document.querySelector('.handle_comment').value;
            if (comment.trim() === '') {
                alert('Please enter handled details');
